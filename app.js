...
function parseYMD(s){if(s instanceof Date){return {y:s.getFullYear(),m:s.getMonth()+1,d:s.getDate()}}const parts=String(s||'').split('-');return {y:Number(parts[0]||0),m:Number(parts[1]||1),d:Number(parts[2]||1)}}
function dateFromStr(s){const {y,m,d}=parseYMD(s);return new Date(y,Math.max(0,(m||1)-1),d||1)}
function ym(d){const dt=d instanceof Date?d:dateFromStr(d);return dt.getFullYear()+"-"+String(dt.getMonth()+1).padStart(2,'0')}
function fmtDate(d){const dt=d instanceof Date?d:dateFromStr(d);const y=dt.getFullYear();const m=String(dt.getMonth()+1).padStart(2,'0');const da=String(dt.getDate()).padStart(2,'0');return `${y}-${m}-${da}`}
...
async function save(){const iv=crypto.getRandomValues(new Uint8Array(12));const data=new TextEncoder().encode(JSON.stringify(state.db));const ct=await crypto.subtle.encrypt({name:'AES-GCM',iv},state.key,data);const blob=new Uint8Array(ct);const b64=btoa(String.fromCharCode(...iv)+String.fromCharCode(...blob));localStorage.setItem('money.enc',b64);localStorage.setItem('money.json',JSON.stringify(state.db));if(localStorage.getItem('gh.auto')==='1'&&localStorage.getItem('gh.token')){try{await saveRemote()}catch(e){console.warn('Auto-sync falhou:',e.message)}}}
async function load(pass){state.key=await deriveKey(pass);const b64=localStorage.getItem('money.enc');const plain=localStorage.getItem('money.json');if(!b64){if(plain){try{state.db=JSON.parse(plain);await save();return true}catch{}}state.db=emptyDb();await save();return true}try{const raw=atob(b64);const iv=new Uint8Array([...raw.slice(0,12)].map(c=>c.charCodeAt(0)));const ct=new Uint8Array([...raw.slice(12)].map(c=>c.charCodeAt(0)));const pt=await crypto.subtle.decrypt({name:'AES-GCM',iv},state.key,ct);const txt=new TextDecoder().decode(pt);state.db=JSON.parse(txt);if(!state.db.invest)state.db.invest={assets:[],ops:[]};return true}catch(e){if(plain){try{state.db=JSON.parse(plain);await save();return true}catch{}}return false}}
...
function statementTotalForMonth(month,closing){const start=dateFromStr(month+'-01');const startWindow=new Date(start);startWindow.setDate(closing+1);const endWindow=new Date(start);endWindow.setMonth(endWindow.getMonth()+1);endWindow.setDate(closing);const items=state.db.cardEntries.filter(e=>{if(e.debitDate){return ym(e.debitDate)===month}const d=dateFromStr(e.date);return d>=startWindow&&d<=endWindow});const total=items.reduce((s,i)=>s+i.amount,0);return {total,items}}
...
ui.entry.invoicePaid.onchange=()=>{const month=ui.entry.filterMonth.value||ym(new Date());const paid=ui.entry.invoicePaid.checked;state.db.cardStatementsPaid[month]=paid;const closing=Number(ui.card.closingDay.value||state.db.settings.cardClosingDay);const stmt=statementTotalForMonth(month,closing);stmt.items.forEach(e=>{e.paid=paid});save().then(()=>{renderDashboard();renderCard();renderBudget()})}
...
ui.lock.unlockBtn.onclick=async()=>{const ok=await load(ui.lock.password.value);if(ok){if(localStorage.getItem('gh.auto')==='1'&&localStorage.getItem('gh.token')){try{await loadRemote(true)}catch{}}ui.lock.overlay.style.display='none';renderAll()}else{ui.lock.password.value=''} }
ui.lock.setupBtn.onclick=()=>{ui.lock.newPassword.style.display='block';ui.lock.setupConfirmBtn.style.display='inline-block'}
ui.lock.setupConfirmBtn.onclick=async()=>{const pass=ui.lock.newPassword.value;if(!pass)return;state.key=await deriveKey(pass);const plain=localStorage.getItem('money.json');if(!state.db){try{state.db=plain?JSON.parse(plain):emptyDb()}catch{state.db=emptyDb()}}await save();ui.lock.overlay.style.display='none';renderAll()}
...
// GitHub sync (server persistence)
ui.io.syncSaveBtn=document.getElementById('syncSaveBtn')
ui.io.syncLoadBtn=document.getElementById('syncLoadBtn')
const GH_OWNER='ravarmes'
const GH_REPO='money'
const GH_PATH=localStorage.getItem('gh.path')||'money.enc'
function getGhToken(){let t=localStorage.getItem('gh.token');if(!t){t=prompt('Informe seu GitHub Personal Access Token (escopo repo)');if(t)localStorage.setItem('gh.token',t)}return t}
async function ghGetContent(path,token){const url=`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${encodeURIComponent(path)}`;const res=await fetch(url,{headers:{'Accept':'application/vnd.github.v3+json','Authorization':`Bearer ${token}`}});if(res.status===404)return null;const j=await res.json();return j}
async function ghPutContent(path,contentB64,token,sha){const url=`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${encodeURIComponent(path)}`;const body={message:'Sync money.enc',content:btoa(contentB64),sha};const res=await fetch(url,{method:'PUT',headers:{'Accept':'application/vnd.github.v3+json','Authorization':`Bearer ${token}`},body:JSON.stringify(body)});if(!res.ok)throw new Error('Falha ao salvar no GitHub');return res.json()}
async function saveRemote(){const token=getGhToken();if(!token)return;const enc=localStorage.getItem('money.enc')||'';const existing=await ghGetContent(GH_PATH,token);const sha=existing?.sha;await ghPutContent(GH_PATH,enc,token,sha)}
async function loadRemote(silent){const token=getGhToken();if(!token)return;const existing=await ghGetContent(GH_PATH,token);if(!existing){if(!silent)alert('Nenhum arquivo remoto encontrado');return}const raw=atob((existing.content||'').replace(/\n/g,'').trim());localStorage.setItem('money.enc',raw.trim());const ok=await load(ui.lock.password.value||'');if(ok){renderAll()}else{if(!silent)alert('Senha incorreta para os dados remotos')}}
if(ui.io.syncSaveBtn){ui.io.syncSaveBtn.onclick=()=>{save().then(()=>saveRemote()).then(()=>alert('Dados salvos no servidor')).catch(()=>alert('Falha ao salvar no servidor'))}}
if(ui.io.syncLoadBtn){ui.io.syncLoadBtn.onclick=()=>{loadRemote()}}
ui.io.syncAutoChk=document.getElementById('syncAutoChk')
if(ui.io.syncAutoChk){ui.io.syncAutoChk.checked=localStorage.getItem('gh.auto')==='1';ui.io.syncAutoChk.onchange=()=>{localStorage.setItem('gh.auto',ui.io.syncAutoChk.checked?'1':'0');if(ui.io.syncAutoChk.checked&&!localStorage.getItem('gh.token'))getGhToken()}}
...
function drawDonut(canvasId,pairs){const c=document.getElementById(canvasId);if(!c||!pairs.length)return;const w=c.width=Math.floor(c.clientWidth||300);const h=c.height=Math.floor(c.clientHeight||140);const ctx=c.getContext('2d');ctx.clearRect(0,0,w,h);const legendW=Math.min(200,Math.floor(w*0.35));const margin=16;const chartW=w-legendW-margin*3;const cx=margin+chartW/2;const cy=h/2;const r=Math.min(chartW,h)/2-18;const sum=pairs.reduce((s,p)=>s+p.v,0)||1;let start=0;const cols=['#22d3ee','#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#14b8a6'];pairs.forEach((p,i)=>{const ang=(p.v/sum)*Math.PI*2;ctx.beginPath();ctx.moveTo(cx,cy);ctx.arc(cx,cy,r,start,start+ang);ctx.closePath();ctx.fillStyle=cols[i%cols.length];ctx.fill();start+=ang});ctx.globalCompositeOperation='destination-out';ctx.beginPath();ctx.arc(cx,cy,r*0.6,0,Math.PI*2);ctx.fill();ctx.globalCompositeOperation='source-over';const lx=margin*2+chartW;const ly=margin;const isLight=document.documentElement.getAttribute('data-theme')==='light';const textCol=isLight?'#0f172a':'#e5e7eb';ctx.textAlign='left';ctx.font='13px Inter';for(let i=0;i<pairs.length;i++){const col=cols[i%cols.length];const y=ly+i*18;ctx.fillStyle=col;ctx.fillRect(lx,y,14,14);ctx.fillStyle=textCol;ctx.fillText(pairs[i].name,lx+20,y+12)}}
