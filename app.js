...
async function ghGetContent(path,token){const url=`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${encodeURIComponent(path)}`;const res=await fetch(url,{headers:{'Accept':'application/vnd.github+json','Authorization':`Bearer ${token}`}});if(res.status===404)return null;const j=await res.json();return j}
async function ghPutContent(path,contentB64,token,sha){const url=`https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${encodeURIComponent(path)}`;const body={message:'Sync money.enc',content:btoa(contentB64),sha};const res=await fetch(url,{method:'PUT',headers:{'Accept':'application/vnd.github+json','Authorization':`Bearer ${token}`},body:JSON.stringify(body)});if(!res.ok)throw new Error('Falha ao salvar no GitHub');return res.json()}
...
async function loadRemote(silent){const token=getGhToken();if(!token)return;const existing=await ghGetContent(GH_PATH,token);if(!existing){if(!silent)alert('Nenhum arquivo remoto encontrado');return}const b64=(existing.content||'').replace(/\n/g,'').trim();localStorage.setItem('money.enc',b64);const ok=await load(ui.lock.password.value||'');if(ok){renderAll()}else{if(!silent)alert('Senha incorreta para os dados remotos')}}
