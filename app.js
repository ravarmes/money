...
async function loadRemote(){const token=getGhToken();if(!token)return;const existing=await ghGetContent(GH_PATH,token);if(!existing){alert('Nenhum arquivo remoto encontrado');return}const content=atob((existing.content||'').replace(/\n/g,''));localStorage.setItem('money.enc',content.trim());const ok=await load(ui.lock.password.value||'');if(ok){renderAll()}else{alert('Senha incorreta para os dados remotos')}}
