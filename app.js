...
const GH_REPO='money'
const GH_PATH=localStorage.getItem('gh.path')||'data/money.enc'
...
async function saveRemote(){const token=getGhToken();if(!token)return;await save();const enc=localStorage.getItem('money.enc')||'';const existing=await ghGetContent(GH_PATH,token);const sha=existing?.sha;await ghPutContent(GH_PATH,enc,token,sha)}
async function loadRemote(silent){const token=getGhToken();if(!token)return;const existing=await ghGetContent(GH_PATH,token);if(!existing){if(!silent)alert('Nenhum arquivo remoto encontrado');return}const content=atob((existing.content||'').replace(/\n/g,''));localStorage.setItem('money.enc',content.trim());const ok=await load(ui.lock.password.value||'');if(ok){renderAll()}else{if(!silent)alert('Senha incorreta para os dados remotos')}}
if(ui.io.syncSaveBtn){ui.io.syncSaveBtn.onclick=()=>{saveRemote().then(()=>alert('Dados salvos no servidor'))}}
